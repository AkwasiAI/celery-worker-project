...
# from google.generativeai.types import HarmCategory, HarmBlockThreshold # If needing safety settings
...
# --- Prompt Templates ---
# Prompts are kept as they are, assuming Gemini will interpret them well.
...
Based on the provided Orasis Base Principles, Executive Summary Detailed Instructions, LLM News Corpus, and the Previous Portfolio, please generate a new draft for the Executive Summary.
...
2.  Aim to retain roughly 60% of the assets/themes from the 'Previous Portfolio' if their rationale still holds given the 'LLM News Corpus'. Introduce approximately 40% new or significantly re-weighted positions.
3.  Ensure all allocation percentages sum to 100.0%.
4.  Adhere to George's specified time horizon distribution.
5.  Provide clear, forward-looking rationales in the narrative.
6.  Output the ENTIRE executive summary including the narrative, the Markdown table, and the hidden JSON block for portfolio positions (<!-- PORTFOLIO_POSITIONS_JSON: [your JSON array here] -->), all as a single string. DO NOT ADD ANYTHING ELSE! YOUR OUTPUT WILL BE PROCESSED DIRECTLY.
...
Please review the following "Portfolio Proposer's Draft" in the context of the "LLM News Corpus".
...
**Your Task:**
1.  Critique the narrative: Is it logical, consistent with the proposed portfolio and the market outlook from the LLM News Corpus?
2.  Critique the portfolio positions: Are the rationales sound? Are there obvious omissions or questionable inclusions given the news? Does it align with Orasis's investment thesis and George's preferences (risk, themes)?
...
3.  **LLM News Corpus (for context):**
    {llm_corpus_content}

4.  **Previous Portfolio (for 60/40 consistency check):**
    {previous_portfolio_json_str}

5.  **Orasis Base Principles & Executive Summary Requirements (for validation - these are already fully formatted with dates etc.):**
    {base_system_prompt_content}
    {executive_summary_detailed_prompt_content}

**Your Tasks:**
1.  **Assess:** Evaluate the Proposer's draft against the Critic's feedback and all Orasis requirements (preferred tickers, 10-15 positions, allocation sum 100%, time horizon distribution, 60/40 consistency with previous portfolio, markdown and hidden JSON format).
...
def extract_structured_parts_from_llm_output(content: str) -> ProposerDraft | None:
    ...
    if match:
        ...
        summary_markdown = content.strip() # The full content is the summary
        try:
            cleaned_json_str = _clean_json_text(positions_json_str)
            # Attempt to load to validate, but store the original cleaned string from the comment
            ...
            # Return the full markdown and a valid empty JSON string if parsing fails
...
def portfolio_proposer_node(state: PortfolioGenerationState) -> Dict[str, Any]:
    ...
    user_prompt = PROPOSER_USER_PROMPT_TEMPLATE.format(
        ...
        llm_corpus_content=state["llm_corpus_content"][:30000], # Gemini 1.5 Pro has a large context window
        previous_portfolio_json_str=json.dumps(state["previous_portfolio_data"], indent=2),
        cio_revision_instructions=cio_instructions
    )
    # For Gemini, a single HumanMessage with clear instructions is often sufficient.
    # SystemMessage can be used to set a persistent persona if desired over multiple turns,
    # but for single-shot complex tasks, including persona in HumanMessage can work.
    # Let's try with System + Human for consistency with your previous structure.
    messages = [
    ...
    try:
        ...
        log.debug(f"Proposer Gemini LLM Raw Output (first 500 chars):\n{raw_llm_output[:500]}")
    ...
    if not parsed_draft: 
        log.error("Proposer extract_structured_parts_from_llm_output returned None unexpectedly.")
        parsed_draft = ProposerDraft(summary_markdown="Critical Error: Proposer draft parsing failed.", portfolio_positions_json_str="[]")
    
    current_scratchpad = state.get("portfolio_scratchpad", [])
    new_scratchpad_entry = {
        "actor": f"PROPOSER (Iter {current_run_iteration_number})",
        "output_markdown_preview": parsed_draft.summary_markdown[:500] + "...", # Log preview
...
def portfolio_critic_node(state: PortfolioGenerationState) -> Dict[str, Any]:
    ...
    if not proposer_draft_md:
        ...
        try:
            ...
            log.info(f"Critic feedback received (first 500 chars):\n{critique[:500]}")
        except Exception as e:
...
def cio_judge_node(state: PortfolioGenerationState) -> Dict[str, Any]:
    ...
    user_prompt = CIO_USER_PROMPT_TEMPLATE.format(
        ...
        llm_corpus_content=state["llm_corpus_content"][:15000], # CIO might need less of full corpus
        previous_portfolio_json_str=json.dumps(state["previous_portfolio_data"], indent=2),
        base_system_prompt_content=state["base_system_prompt_fully_formatted"],
        executive_summary_detailed_prompt_content=state["exec_summary_detailed_prompt_fully_formatted"]
    )
    ...
    try:
        ...
        log.info(f"CIO decision/instructions received (first 500 chars):\n{cio_decision[:500]}")
    except Exception as e:
...
def decide_next_step(state: PortfolioGenerationState) -> str:
    ...
    if "FINAL_PROPOSAL_APPROVED" in cio_decision:
        ...
        log.warning(f"CIO decision unclear or unhandled: '{cio_decision[:100]}...'.")
        if iterations_completed < max_allowed_iterations:
...
def finalize_output_node(state: PortfolioGenerationState) -> Dict[str, Any]:
    ...
    # Prioritize using the proposer's last draft if CIO didn't explicitly provide new text
    # or if max iterations were reached.
    last_proposer_draft = state.get("proposer_draft_markdown")

    if "FINAL_PROPOSAL_APPROVED" in cio_decision:
        parts = cio_decision.split("FINAL_PROPOSAL_APPROVED", 1)
        # Check if CIO provided substantial text after "FINAL_PROPOSAL_APPROVED"
        if len(parts) > 1 and parts[1].strip() and len(parts[1].strip()) > 100: # Arbitrary length check
    ...
    # Include date in scratchpad filename to avoid overwriting from different runs
...
def get_portfolio_generation_graph_gemini() -> StateGraph: # Renamed to avoid conflict if old graph is loaded
...
async def generate_portfolio_executive_summary( # Renamed function
    llm_corpus_content: str,
    previous_portfolio_data: Dict[str, Any],
    ...
    logging.getLogger("google.generativeai").setLevel(logging.INFO) # Adjust Gemini SDK logging
    logging.getLogger("google.api_core").setLevel(logging.INFO)
    
    global LLM_CLIENT, CIO_LLM_CLIENT

    if not google_api_key:
        log.critical("Google API key not provided to generate_portfolio_executive_summary.")
    ...
    LLM_CLIENT = ChatGoogleGenerativeAI(
        ...
        # Add safety_settings or convert_system_message_to_human if needed
    )
    ...
    app = get_portfolio_generation_graph_gemini() # Use the Gemini-specific graph
    ...
    try:
        # LangGraph's stream is synchronous by default if nodes are sync.
        # If you made nodes async, you would use `async for`.
        for step_output in app.stream(initial_state, {"recursion_limit": graph_recursion_limit}):
            ...
            log.debug(f"Graph step executed: {node_name_executed}, current state keys: {list(step_output[node_name_executed].keys())}")
        ...
        return {
            "final_summary_markdown": final_summary_md,
            "portfolio_scratchpad": scratchpad,
            "final_portfolio_positions_json_str": "[]"
        }
    ...
    if not final_run_state or "final_executive_summary_md" not in final_run_state:
        ...
        return {
            "final_summary_markdown": "Error: Portfolio generation process completed without a final summary.",
    ...
    # Clean potential markdown fences from the final summary output
    ...
    return {
        "summary": final_summary_output_md, # Changed key to 'summary' to match your news agent's output
        "portfolio_scratchpad": final_run_state.get("portfolio_scratchpad", []) if final_run_state else [],
        "portfolio_positions": final_portfolio_json_str # Changed key to 'portfolio_positions'
    }